...
class JoinDataPreparator:
    ...
    def generate_n_samples_with_incremental_part(self, sample_size, post_sampling_factor=30, single_table=None, relationship_list=None,
        ...
        sample_size_estimate, full_join_size = self._size_estimate(single_table=single_table,
                                                                   relationship_list=relationship_list,
                                                                   min_start_table_size=min_start_table_size)
        logging.debug(f"generate_n_samples_with_incremental_part(sample_size={sample_size}, single_table={single_table}, "
                      f"relationship_list={relationship_list}, sample_size_estimate={sample_size_estimate}, "
                      f"incremental_learning_rate={incremental_learning_rate}, incremental_condition={incremental_condition})")
        # Sampling of join necessary
        sample_rate = 1.0
        if sample_size_estimate > sample_size:
            sample_rate = min(sample_size / sample_size_estimate * post_sampling_factor, 1.0)
        ...
        if incremental_learning_rate > 0:
            ...
            logging.debug(f"split position for dataset: {split_position} (full length: {full_size}, "
                          f"incremental_rate: {incremental_learning_rate})")
            df_learn_samples = df_full_samples.iloc[0:split_position, :]
            df_inc_samples = df_full_samples.iloc[split_position:, :]
        elif incremental_condition is not None:
            import re
            column, value = re.split(" *[<] *", incremental_condition)
            if value.isdigit():
                value = int(value)
            if value is not None:
                df_learn_samples = df_full_samples[df_full_samples[column] < value]
                df_inc_samples = df_full_samples[df_full_samples[column] >= value]
                logging.info(f"splitting dataset into {len(df_learn_samples)}:{len(df_inc_samples)} parts, "
                             f"according to condition ({incremental_condition}), "
                             f"incremental_rate: {100.0*len(df_inc_samples)/len(df_full_samples)}% @@@")
            else:
        ...
        logging.info(f"split full sample dataset into parts: initial learning size: {len(df_learn_samples)}, "
                     f"incremental: {len(df_inc_samples)}")
    ...
    def generate_join_sample(self, single_table=None, relationship_list=None, min_start_table_size=1, sample_rate=1,
        ...
        logging.debug(f"generate_join_sample(single_table={single_table}, relationship_list={relationship_list}, "
                      f"split_condition={split_condition})")
        if single_table is not None:
